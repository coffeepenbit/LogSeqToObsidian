...
def main():
    ...
    for fname in os.listdir(old_pages):
        ...
        if os.path.isfile(fpath):
            if is_markdown_file(fpath) or is_org_file(fpath):
                ...
                logging.warning(f"Not markdown or org page ({fpath=})")
    # Second loop: for each new file, reformat its content appropriately
    for fpath in new_paths:
        logging.debug(f"({new_paths=}")
        if is_markdown_file(fpath):
            format_markdown_file(args, fpath, new_to_old_paths, old_pagenames_to_new_paths)
        elif is_org_file(fpath):
            format_org_file(args, fpath, new_to_old_paths, old_pagenames_to_new_paths)


def format_markdown_file(args, fpath, new_to_old_paths, old_pagenames_to_new_paths):
    global ORIGINAL_LINE, INSIDE_CODE_BLOCK
    logging.info(f"Formatting markdown file: ({fpath=})")
    newlines = []
...
def is_org_file(fpath: str) -> bool:
    return os.path.splitext(fpath)[-1].lower() == ".org"
...
def get_namespace_hierarchy(fname: str, args) -> list[str]:
...
def update_links_and_tags(line: str, name_to_path: dict, curr_path: str, args) -> str:
